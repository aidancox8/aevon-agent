...
- **Signal loop (reworked 2026-07-09):** repeat-visitors.yml DISABLED (user wants no Resend self-emails; the [Aevon SIGNAL] digest is dead; repeat-visitors.js stays in the repo unused, re-enable via gh workflow enable if ever needed). Signals now live in the cloud routine "Aevon morning replies briefing" (trig_016e43ep7bHUmvTArah9C2fc, weekdays 8am PT, claude-sonnet-5, Gmail+Calendar connectors): reads inbox, drafts replies (NEVER sends), queries Supabase REST directly for last-24h clicked/interested/complained events (extracts the sb_publishable_ key from the public CRM page at runtime; Supabase is not an available routine connector; approach verified working 2026-07-09), applies bot rules (X11/Linux/Headless UA, Chrome <130, click <10min after send, 3+ device families = scanner), watches Vickers + Jean lead IDs, posts 9am calendar-event briefing. [Aevon ALERT] failure emails KEPT (rare, breakage-only). GitHub cron can silently SKIP under load (not a failure, no alert) — caught one 2026-06-08.
