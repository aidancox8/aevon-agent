...
- **Signal loop (reworked 2026-07-09, fixed 2026-07-10):** repeat-visitors.yml DISABLED (no more Resend self-emails; [Aevon SIGNAL] dead). First direct-REST attempt FAILED: the cloud routine's sandbox network policy blocks supabase.co AND github.io (403 on CONNECT); raw.githubusercontent.com IS reachable. Fix: signals-digest.yml (weekdays 14:35 UTC / 7:35am PT) runs signals-digest.js which queries Supabase, applies bot rules (Linux/headless UA, Chrome <130, click <10min after send, 3+ device families = scanner, 30-min session collapse), and commits signals/latest.md (72h window on Mondays to cover weekends). The routine "Aevon morning replies briefing" (trig_016e43ep7bHUmvTArah9C2fc, weekdays 8:05am PT, claude-sonnet-5, Gmail+Calendar) reads that file via raw.githubusercontent.com, checks generated_at staleness, drafts replies (NEVER sends), posts 9am calendar-event briefing. [Aevon ALERT] failure emails KEPT (breakage-only). GitHub cron can silently SKIP under load — caught one 2026-06-08. Also: personalize.yml had been ending as 6h "cancelled" daily (Jul 8-10, silent, no alert since cancelled != failure); scheduled runs now capped --limit 450 + timeout-minutes 330 so failures alert properly.
...
**Email signature rule (2026-07-09):** Gmail does NOT render SVG images. All email signatures must use https://aevon.ca/logo-email.png (80px PNG), never logo.svg. Signature block: logo img 40x40 + "Aidan" (first name only, decided 2026-08-03) + aidan@aevon.ca + Book a call link.
