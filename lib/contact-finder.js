...
const GENERIC = new Set([
  ...
  'fundraising', 'rentals', 'leasing', 'volunteer', 'membership', 'events',
  'donations', 'marketing', 'media', 'press', 'shop', 'store', 'returns',
...
// True if a local-part looks like a scrape artifact rather than a real mailbox.
// Mirrors the sender's emailRisk guard so bad addresses are rejected at extraction
// time, not just at send time. These have real domains but dead mailboxes.
function looksLikeArtifact(local) {
  if (!local) return true;
  if (/[a-z]+(info|contact|reception|support|enquir|inquir|bookkeeping|customerservice|frontdesk|webmaster|hello)/.test(local)) return true;
  if (/^(info|hello|contact|tools|appointment|number|email|clientcare|customerservice|enquir|inquir|support|sales|admin|office|reception)[._-]/.test(local)) return true; // role word + separator + location/junk: "info-burnaby", "clientcare.surrey", "contact.us"
  if (/^[a-z]{1,2}[._-](info|contact|support|sales|hello|reception|admin|office)/.test(local)) return true;
  // role word as the trailing component: "today.info", "abc-contact" — scraper glue, dead mailbox
  if (/[._-](info|contact|support|reception|admin|office|enquir|inquir)$/.test(local)) return true;
  if (local.includes('/')) return true;              // URL captured as an address
  if (/\d{3}[-.]\d{3}/.test(local)) return true;     // embedded phone number
  if (/^\d{2,}[a-z]/.test(local)) return true;       // digit-prefix: "604info"
  if (/^\d{3}[-.]\d{3,4}/.test(local)) return true;  // phone-prefix
  if (local.includes('%')) return true;              // url-encoded junk
  if (local.length > 40) return true;                // concatenated blob
  return false;
}

function collectEmails($) {
  const found = [];
  // mailto: links are the clean, authoritative source.
  $('a[href^="mailto:"]').each((_, el) => {
    const addr = ($(el).attr('href') || '').replace('mailto:', '').split('?')[0].trim().toLowerCase();
    if (addr.includes('@')) found.push(addr);
  });
  // Body-text scan: the ROOT cause of glued artifacts was $('body').text(), which
  // concatenates adjacent elements with NO separator ("Contact"+"info@x" became
  // "contactinfo@x"). Strip scripts/styles, replace every tag with a SPACE, and
  // decode nbsp, so visible text emails stay clean and words can no longer glue.
  let html = $('body').html() || '';
  html = html
  ...
  // Drop asset filenames, vendor noise, and any residual scrape artifacts.
  return found.filter(e =>
    /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(e)
    && !e.includes('%')                                  // url-encoded junk anywhere (incl. domain: "bpa.ca%20%20")
    && !/\.(png|jpg|jpeg|gif|svg|webp)$/.test(e)
    && !/sentry|wixpress|example\.com/.test(e)
    && !looksLikeArtifact(e.split('@')[0])
  );
}
...
// Pages most likely to describe WHAT a business does (for personalization).
function contextPageUrls($, baseUrl) {
  const urls = new Set();
  const wanted = /service|about|what-we-do|solutions|practice|expertise|industr|capabilit|product|how-it-works|why-/i;
  $('a[href]').each((_, el) => {
    const href = $(el).attr('href');
    if (!href || wanted.test(href) === false) return;
    try {
...
// Pull readable, meaningful copy from a page: headings + paragraphs + list items,
// skipping nav/footer/boilerplate. Returns concatenated text.
function readableText($) {
  $('script, style, noscript, nav, footer, header, form, svg').remove();
  var parts = [];
  $('h1, h2, h3, li, p').each(function (_, el) {
    var t = $(el).text().replace(/\s+/g, ' ').trim();
    // keep substantive lines, drop menu/cookie/boilerplate noise
    if (t.length >= 25 && t.length <= 320 && !/cookie|privacy policy|all rights reserved|©|terms of (use|service)/i.test(t)) {
      parts.push(t);
    }
  });
  return parts;
...
/**
 * Scrape a site for PERSONALIZATION context (what they do, services, specialties).
 * Homepage + up to 3 services/about pages. Returns a deduped, content-rich text
 * block (~2500 chars) far richer than a 900-char homepage grab.
 */
async function scrapeContext(websiteUrl) {
  ...
  const seen = new Set();
  const lines = [];
  function add(arr) { arr.forEach(function (t) { var k = t.toLowerCase(); if (!seen.has(k)) { seen.add(k); lines.push(t); } }); }

  add(readableText($home));
  const subs = contextPageUrls($home, websiteUrl);
  for (const url of subs) {
    if (lines.join(' ').length > 2400) break;
    const $p = await fetchPage(url);
    if ($p) add(readableText($p));
  }

  const text = lines.join(' • ').slice(0, 2500).trim();
  return text || null;
}

module.exports = { findContact, classifyEmail, scrapeContext };
