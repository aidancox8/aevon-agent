...
async function fetchPage(url) {
  try {
    const res = await axios.get(url, { timeout: 8000, headers: { 'User-Agent': UA }, maxRedirects: 3 });
...
// Strip label words that get glued to the front of an address in body text,
// e.g. "emailrod@x.com" -> "rod@x.com", "e-mailjane@x.com" -> "jane@x.com".
function cleanEmail(addr) {
  let e = addr.trim().toLowerCase().replace(/^mailto:/, '');
  const local = e.split('@')[0];
  const m = local.match(/^(e-?mail|contact|sendto|mailto)([a-z].*)$/);
  if (m) e = m[2] + '@' + e.split('@').slice(1).join('@');
  return e;
}
