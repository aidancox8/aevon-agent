...
/**
 * `fallbackKey`: a second Gemini key on a different project. The agent key sits on a billed
 * project whose prepaid credit ran out on 2026-09-10 and every model 429'd, while the main key's
 * free tier was answering fine. A dead account is a per-key fact, not a per-Gemini fact, so try
 * the other key before leaving Gemini for OpenRouter's free models, which wrote worse copy.
 */
function createGenerate(apiKey, fallbackKey) {
  ...
  return async function generate(prompt, modelIndex = 0, attempt = 0) {
    if (fallbackGen) return fallbackGen(prompt);
    ...
    try {
      ...
      if (dead) {
        if (fallbackKey && fallbackKey !== apiKey) {
          if (!fallbackGen) {
            console.warn(`${dead} on this key. Switching to the fallback Gemini key for the rest of this run.`);
            fallbackGen = createGenerate(fallbackKey);
          }
          return fallbackGen(prompt);
        }
