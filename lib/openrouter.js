...
const OPENROUTER_MODELS = [
  // Second benchmark, 2026-09-05, on the REAL personalizer prompt (3,200 chars, JSON out), one
  // call each, parsed and run through the copy guard. Wall time is what matters here: the
  // reasoning models pass but think for most of a minute.
  'google/gemma-4-31b-it:free',                        // 3.5s PASS; 429s often, and a 429 costs 150ms
  'poolside/laguna-s-2.1:free',                        // 7s PASS
  'inclusionai/ling-3.0-flash-fin:free',               // 11s PASS
  'nvidia/nemotron-3-ultra-550b-a55b:free',            // 12s PASS
  'z-ai/glm-5.2:free',                                 // sub-second when up, 429 in both real-prompt runs
  'inclusionai/ling-3.0-flash-sante:free',             // 31s PASS
  'nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free',// 46s PASS
  'nvidia/nemotron-3-super-120b-a12b:free',            // 54s PASS
  'google/gemma-4-26b-a4b-it:free',                    // 429 in every run so far
  'minimax/minimax-m3:free',                           // 16s, fails the copy guard (writes says: "...")
  'minimax/minimax-m2.7:free',                         // 34s, same
  'openrouter/free',                                   // their router; 5,800 chars of no JSON once
];

const OPENROUTER_MIN_GAP = 3300;   // ~18 requests/min against a 20/min cap
const REQUEST_TIMEOUT = 75000;   // the slow passers take 30-55s; a timeout wastes the call AND the RPM slot
...
/** Content that is not an answer. Reasoning preambles are fine; callers parse past them. */
...
async function callModel(model, prompt) {
  ...
  const { data } = await axios.post(
    ...
    // No max_tokens on purpose: several of these models write their whole chain of thought into
    // `content` before the answer, and a cap of 1,200 cut them off before the JSON. The caller
    // extracts the JSON from whatever precedes it.
